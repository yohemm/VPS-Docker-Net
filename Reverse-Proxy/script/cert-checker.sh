...
do
    ...
    if [ $execution -eq 1 ]; then
        ...
        if [ -n "$sub_domains" ];then
            ...
            certbot certonly -v --webroot --agree-tos --renew-by-default --preferred-challenges http-01 --email vaxelaire.yohem@gmail.com --webroot-path /var/www/certbot --cert-name ${domain} -d ${domain} -n
        else
